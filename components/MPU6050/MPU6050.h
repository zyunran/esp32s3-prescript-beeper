 ...
 *  - 摇动四向 = 按键事件: 左摇=EVT_OK(确认) / 右摇=EVT_LONG_OK(退出), 送入按键队列
 *    上下摇事件码与物理方向反向: 上摇(俯仰-30侧)=EVT_DOWN / 下摇(+40侧)=EVT_UP
 *    PCB 实测校准: 上下摇=俯仰(+40=下摇/-30=上摇); 左右摇=横滚(0→±30); 航向/平移加速度兜底(见 MPU6050.c)
