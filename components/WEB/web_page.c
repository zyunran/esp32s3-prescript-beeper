...
"<div class=row><label>真字 <input id=gdef type=color value='#63A2EF' style='width:40px;height:28px' oninput=\"colorSync('gdef','gdef_text')\"> <input id=gdef_text type=text value='#63A2EF' maxlength=7 style='width:80px' oninput='gdefText()'></label><label>乱码 <input id=ggb type=color value='#3A5A8A' style='width:40px;height:28px' oninput=\"colorSync('ggb','ggb_text')\"> <input id=ggb_text type=text value='#3A5A8A' maxlength=7 style='width:80px' oninput='ggbText()'></label></div>"
...
"function colorSync(src,dst){document.getElementById(dst).value=document.getElementById(src).value;}"
"function gdefText(){var v=document.getElementById('gdef_text').value;if(/^#[0-9a-fA-F]{6}$/.test(v)){document.getElementById('gdef').value=v;}}"
"function ggbText(){var v=document.getElementById('ggb_text').value;if(/^#[0-9a-fA-F]{6}$/.test(v)){document.getElementById('ggb').value=v;}}"
...
"var gd='#'+j.garble.def,gg='#'+j.garble.gb;document.getElementById('gdef').value=gd;document.getElementById('gdef_text').value=gd;document.getElementById('ggb').value=gg;document.getElementById('ggb_text').value=gg;"
...
"try{let j=await (await post('/api/cfg',body)).json();"
"document.getElementById('msg').textContent=j.ok?'✓ 已保存':'保存失败';}"
"catch(e){document.getElementById('msg').textContent='保存失败(检查网络连接)';}}"   /* 400 等非 JSON 响应也提示, 防残留上次的"已保存" */
...
"if(new TextEncoder().encode(t).length>95){document.getElementById('msg2').textContent='太长: 最多95字节(约31个汉字)';return;}"
"try{let j=await (await post('/api/send',{cmd:t})).json();"
"document.getElementById('msg2').textContent=j.ok?'已发送, 设备正在显示':'发送失败';}"
"catch(e){document.getElementById('msg2').textContent='发送失败(设备不在线?)';}}"
...
"devst().catch(function(){});"
"todoLoad().catch(function(){});"
...
"cloudLoad();"
"load().catch(function(){var s=document.getElementById('st');s.className='st no';s.textContent='加载失败(设备不在线?), 请刷新重试';});</script></body></html>";