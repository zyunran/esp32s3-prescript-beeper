...
"async function todoRemind(i){let en=document.getElementById('rem_en_'+i).checked?1:0,hh=+document.getElementById('rem_hh_'+i).value,mm=+document.getElementById('rem_mm_'+i).value;await fetch('/api/todo',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({op:'remind',idx:i,en:en,hh:hh,mm:mm})});todoLoad();}"
