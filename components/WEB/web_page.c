...
"<div class=row><label>推荐配色</label><select id=gpreset onchange=gpreset(this.value)><option value=''>自定义</option></select><span class=hint>点击填充下方真字/乱码颜色</span></div>"
...
"const G={'默认蓝 63A2EF':{def:'63A2EF',gb:'3A5A8A'}};"
"Object.keys(G).forEach((k,i)=>document.getElementById('gpreset').insertAdjacentHTML('beforeend','<option'+(i===0?' selected':'')+'>'+k+'</option>'));"
"function gpreset(k){if(!G[k])return;document.getElementById('gdef').value='#'+G[k].def;document.getElementById('ggb').value='#'+G[k].gb;}"
