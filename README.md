...
一台传呼机（BB 机）造型的桌面个人终端固件：284×76  UI、三键 + 摇动操作、喇叭/蜂鸣音效，整合 **时钟天气 / 指令乱码破译 / 抽卡拼点 / 闹钟倒计时 / 待办 / 答案之书 / 每日神谕推送**；手机或电脑浏览器即可完成**离线配网、全量配置与指令下发**。
...
| **使用者** | 切换当前使用者；不同使用者收不同专属指令 |
| **设置** | 音量/蜂鸣/按键音；息屏/息屏时钟/主题/光标/破译字/摇动/系统信息/初始化 |
| *织机(隐藏)* | 彩蛋：主界面手势（上上下下左右左右）解锁，「纺织时间」时间加速、「纺织记忆」白框滤镜、平衡姿态页 |
