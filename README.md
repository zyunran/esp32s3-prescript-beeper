...
- 硬件适配：蜂鸣器极性翻转为**高电平触发模块**（GPIO15 高=响 低=静，空闲下拉防上电误响），接线表同步
- 音效：神喻/指令/闹钟/待办的乱码破译，结尾**三连急促蜂鸣与破译完成同步收尾**（按剩余字数对齐，长文本不偏移；节奏在 `BUZZER.h` 可调）
- 音效：其余乱码破译（天气/系统信息/联网状态/网页云端下发）**全程静音**；破译中按确认键跳过适用同规则
- 调整：**按键音精简为 关/音频 两档**（移除 蜂鸣/双；旧配置自动归入音频，设备菜单与网页配置页同步）
- 版本：系统信息页版本号实为 git 标签（ESP-IDF git describe），`version.txt` 从未参与构建，相关注释已修正

## v1.20 更新

- 文档：全库注释巡检同步 20+ 处（架构头/组件清单/子菜单顺序/API 清单/配网项名等陈旧注释），中英 README 全面同步至当前功能
- 无功能代码变更
