...
一台传呼机造型的桌面个人终端：284×76 自绘 UI、三键/摇动、蜂鸣音效。  
...
| 模块 | 引脚 |
|------|------|
| LCD ST7789 284×76 | SCL=7 SDA=8 CS=9 RST=10 DC=11 BLK=12 |
| 按键 上/下/OK | 5 / 6 / 4（按下=低，内部上拉） |
| 有源蜂鸣器 | 15（高有效） |
| MAX98357A I2S | BCLK=16 LRC=17 DIN=18 SD=13 |
| DS1302 | CLK=2 DAT=14 CE=21 |
| MPU6050 | SCL=39 SDA=38（软件 I2C） |
| 电池 ADC | 1（ADC1_CH0，1S 分压） |

避开 N16R8 的 GPIO35/36/37（PSRAM）与 strapping 引脚。改引脚：按键在 `components/UI/ui.h`，其余见各组件 `.c` 顶部。
...
2. 手机连热点 → 自动弹出配置页，或开 `http://192.168.4.1/`  
